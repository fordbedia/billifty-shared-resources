			...
			<div class="payment-cell">
					...
					@if($hasBankTransferDetails)
						<div class="payment-details">
							@foreach($bankTransferDetails as $label => $value)
								<div class="payment-detail-row"><span>{{ $label }}:</span><strong>{{ $value }}</strong>
								</div>
							@endforeach
						</div>
					@else
				...
				<div class="lower-section notes-section">
					...
					@if($invoice->notes || $invoice->terms)
						@if($invoice->notes)
							<div class="note-copy">{!! nl2br(e($invoice->notes)) !!}</div>
						@endif
						@if($invoice->terms)
							<div class="note-copy">{!! nl2br(e($invoice->terms)) !!}</div>
						@endif
					@else
						<div class="muted-line">&mdash;</div>
					@endif
